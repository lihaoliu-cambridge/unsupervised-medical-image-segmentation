...
class Dice(object):
    ...
    def dice(self, y_true, y_pred):
        ...
        # If we're doing soft Dice, require prob output, and the dataloaders already is as we need it
...
class Nonbg(object):
    ...
    All dataloaders is aggregated and the (passed) metric is called on flattened true and
    ...
    def loss(self, y_true, y_pred):
        """ prepare a loss of the given metric/loss operating on non-bg dataloaders """
        yt = y_true #.eval()
        ytbg = np.where(yt == 0)
