...
def stack_models(models, connecting_node_ids=None):
    """
    stacks keras model sequentially without nesting the model into layers
        (the nominal behaviour in keras as of 1/13/2018 is to nest model)
    ...
    Parameters:
        models: a list of model, in order of: [input_model, second_model, ..., final_output_model]
    ...
    # go through model 1 onwards and stack with current graph
...
def predict_volumes(models,
                    ...
                    verbose=False):
    """
    Note: we allow model to be a list or a single model.
    ...
    Returns:
    if model isa list of more than one model:
