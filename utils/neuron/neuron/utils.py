...
def interpn(vol, loc, interp_method='linear'):
    ...
    TODO:
        ...
        check out tf.contrib.resampler, only seems to work for 2D dataloaders
    """
    ...
    if interp_method == 'linear':
        ...
        # clip values
        max_loc = [d - 1 for d in vol.size()]
        clipped_loc = [torch.clip(loc[...,d], 0, max_loc[d]) for d in range(nb_dims)]
...
def transform(vol, loc_shift, interp_method='linear', indexing='ij'):
    ...
    This is a spatial transform in the sense that at location [x] we now have the dataloaders from,
    [x + shift] so we've moved dataloaders.
...
def meshgrid(*args, **kwargs):
    ...
    **kwargs:
    ...
    for i in range(len(output)):       
        output[i] = torch.tile(output[i], torch.stack([*sz[:i], 1, *sz[(i+1):]]))
    return output
...
def predict_volume_stack(models,
    ...
    for batch_idx in batch_gen:
        ...
        # pre-allocate all the dataloaders
        if batch_idx == 0:
