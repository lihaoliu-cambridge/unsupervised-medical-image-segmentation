...
class Miccai2018():
    ...
    def mse_loss(self, y_true, y_pred):
        """ reconstruction loss """
        ndims = y_pred.get_shape()[-1] // 2
        yy1 = y_pred[..., 0:ndims]
        yy2 = y_pred[..., ndims:]
        return mse(yy1, yy2)
        # # print(yy1.shape, yy2.shape)
        # return K.mean(K.abs(yy1 - yy2))


    def kl_loss_1(self, y_true, y_pred, myshape=(80, 96, 112)):
        # print(111)
        """
        KL loss
        y_pred is assumed to be D*2 channels: first D for mean, next D for logsigma
        ...
        flow_vol_shape = myshape

        # compute the degree matrix (only needs to be done once)
        # we usually can't compute this until we know the ndims,
        # which is a function of the data
        # if self.D is None:
        #     self.D = self._degree_matrix(flow_vol_shape)
        self.D1 = self._degree_matrix(flow_vol_shape)

        # sigma terms
        sigma_term = self.prior_lambda * self.D1 * tf.exp(log_sigma) - log_sigma
        sigma_term = K.mean(sigma_term)
    ...
    def kl_loss_2(self, y_true, y_pred, myshape=(40, 48, 56)):
        """
        KL loss
        y_pred is assumed to be D*2 channels: first D for mean, next D for logsigma
        D (number of dimensions) should be 1, 2 or 3
        ...
        # if self.D is None:
        #     self.D = self._degree_matrix(flow_vol_shape)
        self.D2 = self._degree_matrix(flow_vol_shape)
        # sigma terms
        sigma_term = self.prior_lambda * self.D2 * tf.exp(log_sigma) - log_sigma
        sigma_term = K.mean(sigma_term)
    ...
    def kl_loss_3(self, y_true, y_pred, myshape=(20, 24, 28)):
        """
        KL loss
        y_pred is assumed to be D*2 channels: first D for mean, next D for logsigma
        D (number of dimensions) should be 1, 2 or 3
        ...
        # if self.D is None:
        #     self.D = self._degree_matrix(flow_vol_shape)
        self.D3 = self._degree_matrix(flow_vol_shape)

        # sigma terms
        sigma_term = self.prior_lambda * self.D3 * tf.exp(log_sigma) - log_sigma
        sigma_term = K.mean(sigma_term)
